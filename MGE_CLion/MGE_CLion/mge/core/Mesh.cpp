...
// Generate a mesh through code instead of using obj files.
// Is costly at high vertice count.
Mesh* Mesh::loadTerrain(int resolutionX, int resolutionZ)
{
	std::cout << "Loading terrain...";

	Mesh* mesh = new Mesh();

	// Divide Vertices and UV in pieces
	// Since vertices works from -1 to 1 we adjust the formula a little to comply to that structure.
	float dividedVertices = 2.0f / ((float)resolutionX - 1.0f);
	float dividedUVs = 1.0f / ((float)resolutionX - 1.0f);
	for (int x = 0; x < resolutionX; x++)
	{
		// Calculate x vertice and x UV
		float vX = dividedVertices * (float)x - 1;
		float uvX = dividedUVs * (float)x;

		for (int z = 0; z < resolutionZ; z++)
		{
			// Calculate z vertice and z UV
			float vZ = dividedVertices * (float)z - 1;
			float uvZ = dividedUVs * (float)z;

			// Apply new x and z.
			glm::vec3 vertex = glm::vec3(vX, 0, vZ);
			mesh->_vertices.push_back( vertex );

			// The terrain is generated flat so normal will always point upwards.
			glm::vec3 normal = glm::vec3(0,1,0);
			mesh->_normals.push_back( normal );

			glm::vec2 uv = glm::vec2(uvX,uvZ);
	...
	for (int x = 0; x < resolutionX - 1; ++x)
	{
		...
		{
			int verticesIndex = x * resolutionZ + z;

			mesh->_indices.push_back(verticesIndex);
			mesh->_indices.push_back(verticesIndex + 1);
			mesh->_indices.push_back(verticesIndex + resolutionZ + 1);

			mesh->_indices.push_back(verticesIndex);
			mesh->_indices.push_back(verticesIndex + resolutionZ + 1);
			mesh->_indices.push_back(verticesIndex + resolutionZ);
		}
	}
...
//	glDrawArrays(GL_TRIANGLES, 0, 8196 * 8196);
	glBindVertexArray(0);
}
